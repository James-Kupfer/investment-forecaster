...
class MacroQAgent(BaseAgent):
    ...
    def run(
        ...
        # Write macroq columns to the forecast row
        if forecast_id and root_db_id:
            ...
            update_forecast_columns(
                ...
                macroq_rationale=(root_node.get("composite_rationale") or "")[:1000],
            )
        return result
    ...
    def _persist_tree(self, result: AgentResult, prompt_version_id: int) -> Optional[int]:
        ...
        # Use a run-scoped suffix so multiple daily runs don't collide on UNIQUE(node_id, macro_date)
        ...
        with db_cursor() as cur:
            for node in nodes:
                ...
                cur.execute(
                    ...
                    (node.get("composite_rationale") or "")[:1000],
                    node.get("rates_signal"),
                    node.get("rates_confidence"),
                    node.get("dxy_signal"),
                    node.get("dxy_confidence"),
                    node.get("vix_signal"),
                    node.get("vix_confidence"),
                    node.get("sector_signal"),
                    node.get("sector_confidence"),
                    (node.get("node_rationale") or "")[:1000],
                    self.model,
                    prompt_version_id,
                )
                row = cur.fetchone()
