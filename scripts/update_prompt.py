...
def main() -> int:
    ...
    parser.add_argument(
        "--agent",
        required=True,
        help="Agent ID (e.g., 'elicitation', 'momentum')",
    )
    parser.add_argument(
        "--version",
        required=True,
        help="Prompt version (e.g., 'v1.1', 'v1.2')",
    )
    parser.add_argument(
        "--text",
        help="Prompt text (if not using --file)",
    )
    parser.add_argument(
        "--file",
        help="Path to file containing prompt text (alternative to --text)",
    )
