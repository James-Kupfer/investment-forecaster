...
def _price_on_date(symbol: str, target_date: date) -> Optional[float]:
    """Return the closing price on or just before target_date."""
    import yfinance as yf
    start = target_date.strftime("%Y-%m-%d")
    # Fetch a small window around the target date
    hist = yf.Ticker(symbol).history(start=start, end=None, period="5d")
    if hist.empty:
        return None
...
def _update_agent_weight(
    ...
    row = cur.fetchone()
    accuracy = 1.0 - brier  # higher is better
    if row:
        n = row[1] + 1
